...
function sassTask() {
  return src(files.sass)
    ...
    .pipe(bs.stream()); // Injects new CSS to the browser (dev mode)
}
...
// Images Task:
// 1. Copies the image files to the dist images folder
function imagesTask() {
  return src(files.images).pipe(dest(folders.imagesDist)); // Copies compressed images to the dist images folder
}
...
function watchTask() {
  ...
  watch(files.js).on('change', series(jsTask, bs.reload)); // Watches the dev JavaScript directory (JS files only)
  watch(files.html).on('change', bs.reload); // Watches the dev folder (HTML files only)
}
...
// Default Task:
export default series(parallel(sassTask, jsTask), watchTask);
